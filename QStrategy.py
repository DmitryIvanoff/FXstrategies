...
class QStrategy(FXStrategy):
    ...
    def __init__(self):
        ...
        self.actions = [
            (0,0,0), #no action
            (1,0,0), #long(buy) open
            (0,1,0), #short(sell) open
            (1,0,1), #long(buy) close
            (0,1,1)  #short(sell) close
        ]
        self.history = []

    def init_state(self, data, time_step):
        """
        :param data: данные
        :return:
        """
    ...
    def get_reward(self, state, action, next_state, time_step, broker, q):
        reward = 0
        if action == -1:  # buy
            if q <= 0.0:
                reward = 0
    ...
    def trade(self, show=False, *params, **kwargs):
        # print(params)
        ...
        Broker_bitfin = None#Backtest('Bitfinex', data_all)
        ...
        for i in range(epochs):
            ...
            state, xdata = self.init_state(data_bitfin, time_step)
            # print(state)
            # print(type(state))
            # print(xdata)
            status = 1
            ...
            while status == 1:
                ...
                for a in self.get_actions(state):
                    ...
                    qval = model.predict(
                        np.concatenate((np.array([state]).reshape(1, 1), np.array([a]).reshape(1, 1)), axis=1),
                        batch_size=1)
                    Q_n.append(qval)
                if (random.random() < epsilon):  # choose random action
                    optimal_action = self.ACTIONS[np.random.randint(0, len(self.ACTIONS))]  # assumes 3 different actions
                else:  # choose best action from Q(s,a) values
                    optimal_action = (np.argmax(Q_n) - 1)
                # optimal_action = (np.argmax(Q_n) - 1)
                next_state, time_step, terminal_state, amount = self.take_action(state, xdata, optimal_action, time_step,
                                                                            Broker_bitfin)
                # if amount <= 0.0:
                #     optimal_action=0
                # print(time_step)
                reward = self.get_reward(state, optimal_action, next_state, time_step, Broker_bitfin, amount)
                if i == epochs - 1:
                ...
                if len(storage) < storage_size:  # if buffer not filled, add to it
                    ...
                    storage.append((np.array([state]).reshape(1, 1), np.array([optimal_action]).reshape(1, 1), reward,
                    ...
                    storage[h] = (np.array([state]).reshape(1, 1), np.array([optimal_action]).reshape(1, 1), reward,
                                  np.array([next_state]).reshape(1, 1))
                    ...
                    for memory in storage:
                        ...
                        for a in self.get_actions(new_state):
                            ...
                            qval = model.predict(np.concatenate((new_state, np.array([a]).reshape(1, 1)), axis=1),
                                                 batch_size=1)
                            Q_n.append(qval)
                        maxQ = np.max(Q_n)
...
if __name__ == '__main__':
    strategy = QStrategy()
    strategy.load_history('EUR_USD',datetime.datetime(2018, 1, 1, 0, 0, 0, 0))
    print(len(strategy.history))